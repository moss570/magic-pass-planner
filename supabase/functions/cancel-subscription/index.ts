...
serve(async (req) => {
  ...
  try {
    ...
    if (subscriptions.data.length === 0) {
      ...
      if (trialSubs.data.length === 0) throw new Error("No active subscription found");
      await stripe.subscriptions.update(trialSubs.data[0].id, { cancel_at_period_end: true });
    } else {
